    ...
    if r.get('code') == 121002:
        r2 = api('POST', f'{BASE}/drive/v1/files/upload_all', token=token,
                 data={'file_name': filename, 'parent_type': 'explorer',
                       'parent_node': parent_token, 'size': str(len(data))},
                 files={'file': (filename, data, 'application/octet-stream')})
        if r2.get('code') == 0:
            return r2['data']['file_token']
        print(f"  ⚠️ 覆盖上传失败 {filename}")
        return None
...
def main():
    ...
    # 创建新根目录 AI课程/
    r = api('POST', f'{BASE}/drive/v1/files/create_folder', token=token,
            json={'name': 'AI课程', 'folder_token': 'QhA7fYs0vlclUIdI1oScGnOen2g'})
    if r.get('code') != 0:
        print(f"❌ 创建根目录失败: {r.get('msg', r)}")
        return
    root_token = r['data']['token']
    print(f"✅ 创建根目录 AI课程/ (token: {root_token})\n")
